...
export const readFileSchema: BaseToolSchema = {
	...
	customDescription: (args: ToolArgs) => {
		...
		if (partialReadsEnabled) {
			schema.parameters[0].items.properties.line_range = {
				name: "line_range",
				type: "string",
...
`
		...
		if (isMultipleReadsEnabled) {
			schema.description += `${isMultipleReadsEnabled ? `- When you need to read more than ${maxConcurrentReads} files, prioritize the most critical files first, then use subsequent read_file requests for additional files` : ""}`
		}
